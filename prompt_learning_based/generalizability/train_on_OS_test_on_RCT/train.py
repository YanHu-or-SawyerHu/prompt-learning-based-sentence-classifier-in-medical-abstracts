...
parser.add_argument("-p", "--percentage_of_training_data", help="how many training data")
parser.add_argument("-t", "--template", help="which template")
parser.add_argument("-C", "--CUDA_VISIBLE_DEVICES", help="which GPU")
args = parser.parse_args()

os.environ['CUDA_VISIBLE_DEVICES'] = str(args.CUDA_VISIBLE_DEVICES)
percentage_of_training_data = float(args.percentage_of_training_data)
...
# read data
data_dir = '../../data/observational_with_title/'
train =  data_dir+'observational_with_title_train.txt'
valid = data_dir+'observational_with_title_valid.txt'
#tes1 = '/collab/yhu5/PICO_sentence_classification/HSLN-Joint-Sentence-Classification/data/RCT_raw_test/PICO_test_raw.txt'
test = '../../data/pubmed-rct/PubMed_20k_RCT_with_title/test.txt'
...
from openprompt.prompts import ManualTemplate
mytemplate = ManualTemplate(tokenizer=tokenizer, text=template_text)
if len(reduced_train_dict) != 0:
  train_dataloader = PromptDataLoader(dataset=dataset["train"], template=mytemplate, tokenizer=tokenizer, tokenizer_wrapper_class=WrapperClass, max_seq_length=512, decoder_max_length=512,batch_size=8,shuffle=True, teacher_forcing=False, predict_eos_token=False, truncate_method="head")
